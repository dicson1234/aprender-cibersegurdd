...
(function(){
  ...
  function normalizeMobilePath(){if(window.innerWidth>700)return;document.querySelectorAll('.serpentine-path .node-wrapper').forEach((node,index)=>{node.style.transform=`translateX(${index%2===0?-24:24}px)`;});}
  function updateMobileNav(){const current=(location.hash||'#dashboard').slice(1);document.querySelectorAll('.mobile-nav-item').forEach(item=>item.classList.toggle('active',(item.getAttribute('href')||'')===`#${current}`));}

  function ensureSharedStyles(){
    if(document.getElementById('cyberlab-final-ui-styles'))return;
    const style=document.createElement('style');style.id='cyberlab-final-ui-styles';style.textContent=`
  ...
  function secureMarkdownPatch(){
    ...
    tutor.parseMarkdown=function(text){
      ...
      source=source.replace(/```([a-z0-9_-]*)\s*\n?([\s\S]*?)```/gi,(m,lang,code)=>{const safe=code.replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));const index=codeBlocks.push(`<pre><code>${safe.trim()}</code></pre>`)-1;return `@@CODE${index}@@`;});
  ...
  function secureHeaderPatch(){
    const app=window.CyberApp;if(!app||app.__safeHeaderPatched)return;
    app.updateHeaderStats=function(){
      ...
      const account=window.CyberAccounts?.getActive?.(),box=document.getElementById('header-user-profile-badge');if(!box||!account)return;
      ...
      if(av){const img=document.createElement('img');img.src=av;img.alt='';img.width=24;img.height=24;img.loading='lazy';img.style.cssText='border-radius:50%;object-fit:cover;border:1px solid var(--accent-cyan);';box.appendChild(img);}else{const span=document.createElement('span');span.textContent=account.avatar||'👤';span.style.fontSize='1.1rem';box.appendChild(span);}
      const name=document.createElement('span');name.textContent=account.username||'Estudiante';name.style.cssText='font-weight:700;font-size:.85rem';box.appendChild(name);
    };
    app.__safeHeaderPatched=true;app.updateHeaderStats();
  }

  function init(){
    setTheme(localStorage.getItem(THEME_KEY)||'dark');ensureSharedStyles();
